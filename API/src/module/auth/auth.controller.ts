...
export class AuthController {
  ...
  async signUp(@Res() response, @Body() signUpPayload: User) {
    const { isValidate, message } =
      this.authServices.signUpValidation(signUpPayload);
    if (isValidate) {
      const user: User = await this.usersServices.findUserByEmail(
        signUpPayload.email,
      );
      if (user) {
  ...
  async forgotPassword(@Res() response, @Body() body: User) {
    ...
    if (user) {
      ...
      throw new BadRequestException(
        'Please Enable MFA for create new password!!',
      );
    }
  ...
  async resetPassword(
    ...
    if (user) {
      ...
      if (isPasswordMatch) {
        ...
        const isUpdate = this.usersServices.updateUser(
          updateUser,
          request.user.id,
        );
        if (isUpdate) {
  ...
  async register(@Res() response, @Req() request) {
    const { otpAuthUrl } = await this.authServices.generateMFASecret(
      request.user,
    );
    const qrCodeLink = await this.authServices.generateQrCodeDataURL(
      otpAuthUrl,
    );
  ...
  async turnOnTwoFactorAuthentication(
    ...
    if (user) {
      ...
      if (isCodeValid) {
        const isTurnOn = await this.usersServices.turnOnTwoFactorAuthentication(
          request.user.id,
        );
        if (isTurnOn) {
          const token = this.authServices.generateToken(user);
